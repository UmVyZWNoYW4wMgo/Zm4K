...
echo "sed -i '/^### $user $exp/,/^},{/d' /etc/xray/json/split.json && systemctl restart xray@split && systemctl restart quota-split && rm -fr /var/log/create/xray/split/${user}.log && rm -fr /etc/xray/limit/ip/xray/split/$user && rm -fr /etc/xray/quota/split/$user" | at now + 60 minutes >/dev/null 2>&1
clear
echo -e "$TEKS"
