...
echo "sed -i '/^### $user $exp/,/^},{/d' /etc/xray/json/grpc.json && systemctl restart xray@grpc && systemctl restart quota-grpc && rm -fr /var/log/create/xray/grpc/${user}.log && rm -fr /etc/xray/limit/ip/xray/grpc/$user && rm -fr /etc/xray/quota/grpc/$user" | at now + 60 minutes >/dev/null 2>&1
clear
echo -e "$TEKS"
